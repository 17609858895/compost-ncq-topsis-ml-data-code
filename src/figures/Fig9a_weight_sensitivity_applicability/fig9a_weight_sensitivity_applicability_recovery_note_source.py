...
def annotate_heatmap(ax, data, fmt="{:.2f}", threshold=0.55):
    for i in range(data.shape[0]):
        for j in range(data.shape[1]):
            ...
            ax.text(j, i, fmt.format(value), ha="center", va="center", fontsize=8.0, color=color)


def format_heatmap_axis(ax, rows, cols, *, xtick_size=9.2, ytick_size=9.6):
...
fig = plt.figure(figsize=(17.6, 15.0), dpi=300)
gs = fig.add_gridspec(
    3,
    2,
    width_ratios=[1.02, 1.18],
    height_ratios=[1.02, 1.0, 1.08],
    wspace=0.42,
    hspace=0.58,
)
...
ax_a.text(-0.04, -0.035, "N", ha="right", va="top", fontsize=11.5, fontweight="bold")
ax_a.text(1.04, -0.035, "C/GHG", ha="left", va="top", fontsize=11.5, fontweight="bold")
ax_a.text(0.5, np.sqrt(3) / 2 + 0.035, "GI", ha="center", va="bottom", fontsize=11.5, fontweight="bold")
...
cbar_a.set_label("Rank percentile of balanced recipe", fontsize=10.0)
cbar_a.ax.tick_params(labelsize=9.2)
...
for cls, color in zip(class_order, colors):
    ...
    for i, val in enumerate(vals):
        if val >= 6:
            ax_b.text(left[i] + val / 2, i, f"{val:.0f}%", ha="center", va="center", fontsize=9.2)
    left += np.asarray(vals)
ax_b.set_xlim(0, 100)
ax_b.set_xlabel("Weight scenarios (%)")
ax_b.set_ylabel("")
ax_b.legend(loc="upper center", bbox_to_anchor=(0.5, 1.18), ncol=4, frameon=False, fontsize=9.4)
...
for y_pos, val in enumerate(ad_plot["Value"]):
    ax_c.text(val + 0.025, y_pos, f"{val:.2f}", va="center", ha="left", fontsize=9.5)
...
for y_pos, val in enumerate(gap_plot["Absolute normalized gap"]):
    ax_d.text(val + 0.01, y_pos, f"{val:.2f}", va="center", ha="left", fontsize=9.2)
...
format_heatmap_axis(ax_e, strategy_short, objective_labels, xtick_size=9.4, ytick_size=9.6)
cbar_e = fig.colorbar(im_e, ax=ax_e, fraction=0.046, pad=0.02)
cbar_e.set_label("Score", fontsize=9.6)
cbar_e.ax.tick_params(labelsize=8.6)
...
for i in range(change_vals.shape[0]):
    for j in range(change_vals.shape[1]):
        ax_f.text(j, i, f"{change_vals[i, j]:+.1f}", ha="center", va="center", fontsize=7.8, color="black")
format_heatmap_axis(ax_f, strategy_short, target_labels, xtick_size=8.8, ytick_size=9.4)
cbar_f = fig.colorbar(im_f, ax=ax_f, fraction=0.034, pad=0.018)
cbar_f.set_label("Favourable change vs typical", fontsize=9.4)
cbar_f.ax.tick_params(labelsize=8.4)

polish_figure(fig, text_scale=0.98)
for ax in [ax_a, ax_b, ax_c, ax_d, ax_e, ax_f]:
    ax.xaxis.label.set_fontsize(12.2)
    ax.yaxis.label.set_fontsize(12.2)
    ax.tick_params(labelsize=10.0)
for ax in [ax_e, ax_f]:
    for text in ax.texts:
        text.set_fontsize(7.8)

save_figure_with_subfigures(fig, OUT_DIR, STEM, [[ax_a, cbar_a.ax], ax_b, ax_c, ax_d, [ax_e, cbar_e.ax], [ax_f, cbar_f.ax]])
plt.close(fig)
